  ...
  : Node("fieldline_node", options)
  {
    ...
    if (!engine_path_.empty()) {
      try {
        ...
        const auto & m = engine_->tensor(mask_name_);
        mask_channels_ = m.shape.size() == 4 ? static_cast<int>(m.shape[1]) : 1;
        mask_h_ = static_cast<int>(m.shape[m.shape.size() - 2]);
        mask_w_ = static_cast<int>(m.shape[m.shape.size() - 1]);
        RCLCPP_INFO(
          get_logger(), "segmentation ready: %dx%d input -> %dx%dx%d mask",
          input_size_, input_size_, mask_channels_, mask_h_, mask_w_);
      } catch (const std::exception & e) {
        RCLCPP_ERROR(
          get_logger(), "failed to load segmentation engine (%s) - using HSV fallback", e.what());
        engine_.reset();
      }
    } else {
      RCLCPP_WARN(
...
private:
  void on_image(const sensor_msgs::msg::Image::ConstSharedPtr & msg)
  {
    ...
    try {
      ...
      if (engine_) {
        segment_trt(cv->image, pixels);
      } else {
        segment_hsv(cv->image, pixels);
      }
      project(pixels, out);
    } catch (const std::exception & e) {
  ...
  void segment_trt(const cv::Mat & bgr, std::vector<cv::Point2f> & pixels)
  {
    ...
    for (int y = 0; y < mask_h_; ++y) {
      for (int x = 0; x < mask_w_; ++x) {
        ...
        pixels.emplace_back(static_cast<float>(sx), static_cast<float>(sy));
      }
    }
  }

  void segment_hsv(const cv::Mat & bgr, std::vector<cv::Point2f> & pixels)
  {
    ...
    pixels.reserve(nonzero_.total());
    for (size_t i = 0; i < nonzero_.total(); ++i) {
  ...
  void project(
    const std::vector<cv::Point2f> & pixels, soccer_msgs::msg::FieldFeatureArray & out) const
  {
    if (pixels.empty()) {return;}
    const size_t step = std::max<size_t>(
      1, pixels.size() / static_cast<size_t>(std::max(1, max_points_)));

    for (size_t i = 0; i < pixels.size(); i += step) {
      const auto p = camera_.project_flat_ground(pixels[i].x, pixels[i].y);
      if (!p) {continue;}                        // above the horizon
      if ((*p)[0] > max_range_m_) {continue;}    // beyond useful range
  ...
  double mask_threshold_ = 0.5, max_range_m_ = 6.0, confidence_ = 0.8;
